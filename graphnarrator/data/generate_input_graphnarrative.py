...
def get_relation(edge_label):
    ...
    if len(edges_combined) == 2:
        ...
        #new_edge_label = new_edge_label.encode('utf-8').strip()
...
def get_data_dev_test(file_, train_cat, dataset):
    ...
    for e in entries:
        ...
        lexs = e.getElementsByTagName('origin')
...
def get_data(file_):
    ...
    for e in entries:
        ...
        mtriples = e.getElementsByTagName('mtriple')
        #print("len(mtriples)=", len(mtriples))
        
        nodes = process_triples(mtriples)
        #print("len(nodes)", len(nodes))
        #print("nodes", nodes)
        lexs = e.getElementsByTagName('origin')
        #print("len(lexs)", len(lexs))
        #print("lexs", lexs)
        if len(lexs) > 0:
            for l in lexs:
                #l = l.firstChild.nodeValue.strip().lower()
